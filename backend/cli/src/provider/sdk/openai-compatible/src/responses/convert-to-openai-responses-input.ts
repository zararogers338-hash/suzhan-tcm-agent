            ...
            switch (part.type) {
              ...
              case "file": {
                if (part.mediaType.startsWith("image/")) {
                  ...
                  if (mediaType === "image/*" || mediaType === "application/octet-stream") {
                    // Detect actual image type from magic bytes in base64 data
                    const b64 = typeof part.data === "string" ? part.data : ""
                    const raw = atob(b64.slice(0, 16))
                    if (raw.charCodeAt(0) === 0x89 && raw.slice(1, 4) === "PNG") mediaType = "image/png"
                    else if (raw.charCodeAt(0) === 0xff && raw.charCodeAt(1) === 0xd8) mediaType = "image/jpeg"
                    else if (raw.slice(8, 12) === "WEBP") mediaType = "image/webp"
                    else if (raw.slice(0, 3) === "GIF") mediaType = "image/gif"
                    else mediaType = "image/png" // safe fallback for Anthropic
                  }
