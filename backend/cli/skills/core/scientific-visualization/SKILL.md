...
description: Creates and audits truthful, accessible, publication-ready scientific figures with Matplotlib, Seaborn or Plotly, covering figure design, multi-panel layouts, uncertainty and missing-data displays, colour and contrast review, image metadata validation and journal export planning. Load alongside figures when a plot must meet a specific journal's requirements or needs an accessibility or export audit. Not for diagrams (use schematics) or illustrations (use generate-image).
summary: "Publication figure library: journal profiles, colorblind-safe palettes, uncertainty displays, metadata and export audits."
category: core
role: workflow
...
metadata:
  ...
  upstream-relationship: vendored with frontmatter adapted
